...
# start-local-env.sh
#
# Startup script for Rebecca AI local manual testing environment (Mac/Linux).
...
mkdir -p .logs
npx -y firebase-tools emulators:start --project rebecca-ai-gal-local > .logs/firebase-emulators.log 2>&1 &
echo -e "\033[0;90m-> Firebase Emulators launching in background (logs in .logs/firebase-emulators.log)...\033[0m"

echo -e "\033[0;33m[4/7] Waiting for Firestore Emulator (port 8080) to bind...\033[0m"
RETRIES=0
MAX_RETRIES=30
READY=false
while [ $RETRIES -lt $MAX_RETRIES ] && [ "$READY" = false ]; do
    sleep 2
    if nc -z 127.0.0.1 8080 2>/dev/null && nc -z 127.0.0.1 9099 2>/dev/null && nc -z 127.0.0.1 9199 2>/dev/null; then
        READY=true
    else
        RETRIES=$((RETRIES+1))
        echo -e "\033[0;90m   Waiting... ($RETRIES/$MAX_RETRIES)\033[0m"
    fi
done

if [ "$READY" = false ]; then
    echo -e "\033[0;31m[ERROR] Firebase Emulators failed to start. Check .logs/firebase-emulators.log\033[0m"
    exit 1
fi
...
if npx ts-node -T apps/dashboard-backend/src/scripts/seed-db.ts; then
    echo -e "\033[0;32m-> Database seeded successfully!\033[0m"
else
...
echo -e "\033[0;32m==========================================================\033[0m"
echo -e "\033[0;33mTo stop all services, run: killall node java\033[0m"
