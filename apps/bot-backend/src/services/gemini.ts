...
const verifyImageRelevance = async (imageCaption: string, postText: string): Promise<boolean> => {
    ...
    try {
        const prompt = `あなたはAIキャラクター「レベッカ」のSNS投稿と添付画像の文脈調和を判定するモデレーターAIです。
画像データベースには「レベッカ自身が様々な衣装・シチュエーションで描かれたイラスト」が登録されています。
以下の「投稿テキスト」に対し、「画像」がレベッカ自身の投稿行動や発言の背景情景として自然に調和しているかを判定してください。
...
※「レベッカが写っていれば何でもよい」という安易な判定は避け、投稿の話題と画像のアクティビティ・場面設定の整合性を厳密に評価してください。
