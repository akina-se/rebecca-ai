...
const generateTimelineSummary = async (prompt: string | string[], previousSummary?: string): Promise<string> => {
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: contentStr,
            config: {
                systemInstruction: 'あなたはAIキャラクターの長期記憶管理システムです。与えられたキャラクター自身の投稿履歴と過去の要約を元に、最近どのようなニュース、技術、社会トレンドや日常のトピックについて言及していたかを客観的に400文字以内で要約してください。言及されたトピックの内容や関心の推移を中心に記述し、語尾や過剰な感情表現の模倣ではなく、対話や思考の知的背景となる文脈を自然な日本語の文章で整理してください（箇条書きや記号は使用しないでください）。',
                maxOutputTokens: 400
            }
        });
