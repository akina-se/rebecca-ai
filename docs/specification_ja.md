...
4. 対象者のプロフィール文をX APIで取得し、Geminiに分析させる（趣味、属性など）。
5. 分析結果をもとに `random_engagement` 用の不意打ちコンテキストプロンプトを構築し、メンション文章を生成。
6. メンション付きでXへ投稿し、対象者を `list_interaction_history` に記録して完了（1ユーザーにつき1回のみ実行）。
