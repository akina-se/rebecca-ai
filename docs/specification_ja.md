...
- **バッチ4：Proactive Talk（自発的なニュース共有と甘やかし / 画像添付）**
  ...
  - **画像添付処理**: 投稿内容と直近のタイムライン要約から、現在の状況や感情を表す「キーワード」を推論する。GCSおよびFirestoreに事前登録された画像群のタグとキーワードを照合し、合致する画像があれば自動で添付してポストする（クールダウン期間による重複回避制御あり）。
...
日間のリプライ上限に達した際、システムはフェイルセーフとして新規リプライ処理を一時停止する。この際、単なるエラー停止とせず、翌朝の定期ポスト等を通じて「今日の返信配給枠は〇〇件よ。私の演算リソースは限られてるんだからね」といったキャラクター性を伴うメッセージを出力させることで、運用上の制約を自然な形でユーザーへ提示する仕様とする。
