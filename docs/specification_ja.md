...
- **バッチ4：Proactive Talk（自発的なニュース共有と甘やかし / 画像添付）**
  - **実行時間**: 毎日 複数回（朝、昼、夜など）
  - **処理**: パブリックなRSSフィード（Yahoo! ニュース等）を取得し、Geminiに「マスターが共感しそうなニュース」を選ばせて100文字以内で甘やかすようなポストを生成させ、Xに自発的に投稿する。
  - **画像添付処理**: 投稿内容と直近のタイムライン要約から、現在の状況や感情を表す「検索クエリ」を推論する。クエリをベクトル化し、GCSおよびFirestoreに事前登録された画像群のキャプションベクトルと照合（コサイン類似度検索）し、意味的に合致する画像があれば自動で添付してポストする（クールダウン期間による重複回避制御あり）。
...
日間のリプライ上限に達した際、システムはフェイルセーフとして新規リプライ処理を一時停止する。この際、単なる無応答とするのではなく、翌朝の定期ポスト等の自発的なメッセージ内で自身の演算リソース（返信可能件数）の制限について言及する設計とし、システム運用上の制約をキャラクター設定（世界観）に組み込んで自然に表現する仕様とする。
