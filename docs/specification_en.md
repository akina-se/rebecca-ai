...
- **Batch 4: Proactive Talk (Spontaneous News Sharing and Image Attachment)**
  ...
  - **Image Attachment**: Infers a "keyword" representing the current emotion or situation based on the generated text and timeline summary. It searches pre-analyzed images in GCS and Firestore using this keyword, and automatically attaches a matching image to the post (with cooldowns to prevent repetition).
...
When the daily reply limit is reached, the system will temporarily halt new reply processing as a fail-safe. To handle this gracefully, the automated morning post on the following day should explicitly mention the limitation as a character trait (e.g., "Today's reply ration is limited to XX. My compute resources aren't infinite, you know!"). This specification ensures that operational constraints are presented naturally to the user without breaking the persona.
