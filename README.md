...
- **Proactive News Deduplication & Autonomous Soliloquy**: Uses deterministic vector cosine similarity against a 30-day historical window to filter out duplicate news stories before generation. When no fresh news is found, or when scheduled directly, the system autonomously transitions into "Soliloquy Mode" to share spontaneous personal reflections, affirmations, and time-of-day greetings without prompt pollution.
- **Proactive Anniversary Post Engine ("◯◯の日")**: Dynamically sources real-world calendar memorial days via an abstract `IAnniversaryProvider` backed by Wikipedia day pages. Injects cultural and lifestyle topics into Rebecca's Gyaru persona, activating thematic visual assets and falling back gracefully to soliloquy mode upon network timeout.
...
- **Generic Web Search Grounding & Multi-Context Dialogue**: Supports unconstrained 1-on-1 private dialogue (`chat`) alongside public X mentions (`reply`). Incorporates Google Search Grounding (`search_web`) to autonomously look up up-to-date information, facts, and answers during conversations without hardcoded news dependencies.
- **Narrative Campaign Event Engine**: Enables temporal, episodic storytelling arcs (e.g. multi-day travel itineraries, seasonal festivals, special anniversary weeks) orchestrated through automated timetable slot dispatching. Features strict date overlap rejection, atomic slot state progression, situational persona anchoring with world-building context, and automatic routine post suppression during active events.
